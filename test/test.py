...
def safe_int(sig):
    """Return int(sig.value) if resolvable, else None."""
    v = sig.value
...
class DigitCapture:
    """Continuously captures digits when latch-enables pulse. Skips X/Z cycles."""

    def __init__(self, dut):
        self.dut = dut
        self.Ht = 0; self.Ho = 0; self.Mt = 0; self.Mo = 0; self.St = 0; self.So = 0
    ...
    async def run(self):
        while True:
            ...
            uo = safe_int(self.dut.uo_out)
            uio = safe_int(self.dut.uio_out)
            if uo is None or uio is None:
                continue  # skip until signals are 0/1 clean
            seg_bus = uo & 0x7F            # {a..g}
            le = uio & 0x3F               # {Ht..So}
            if le & (1 << 0): self.Ht = self._decode(seg_bus)
            if le & (1 << 1): self.Ho = self._decode(seg_bus)
            if le & (1 << 2): self.Mt = self._decode(seg_bus)
            if le & (1 << 3): self.Mo = self._decode(seg_bus)
            if le & (1 << 4): self.St = self._decode(seg_bus)
            if le & (1 << 5): self.So = self._decode(seg_bus)
...
def ui_bit_set(dut, idx: int, v: int):
    cur = ui_get(dut)
    cur = (cur | (1 << idx)) if v else (cur & ~(1 << idx))
    ui_set(dut, cur)

async def press(dut, bit_idx: int, high_ticks: int):
    """Hold a ui_in bit high for 'high_ticks' clock cycles (debounced press)."""
...
async def step_n(dut, bit_idx: int, n: int):
    for _ in range(n):
        await press(dut, bit_idx, 3)   # DEB_LEN = 3
...
async def measure_colon_ticks(dut, expect: int, tol: int = 0) -> int:
    """Count clk cycles between two colon LED toggles. Skips unresolved cycles."""
    # Wait until colon is resolvable
    while safe_int(dut.uo_out) is None:
        await RisingEdge(dut.clk)
    colon = (safe_int(dut.uo_out) >> 7) & 1

    # wait for next edge
    while True:
        await RisingEdge(dut.clk)
        v = safe_int(dut.uo_out)
        if v is None:  # skip X/Z cycles
            continue
        c = (v >> 7) & 1
        if c != colon:
            colon = c
...
async def test_project(dut):
    ...
    # Sped-up "AC tick": one clk cycle = 200 us (5 kHz). Any value works; tests count ticks.
    ...
    # Wait a few cycles for DUT to drive known 0/1s
    for _ in range(10):
        await RisingEdge(dut.clk)
        if safe_int(dut.uo_out) is not None and safe_int(dut.uio_out) is not None:
            break

    # Start digit capture AFTER reset & settle to avoid X/Z conversion errors
    ...
    # ---------------- PPS alignment ----------------------
    ui_bit_set(dut, 5, 0)    # back to 60 Hz
    ...
    prev = ((safe_int(dut.uo_out) or 0) >> 7) & 1
    cnt = 0
    while True:
        await RisingEdge(dut.clk)
        v = safe_int(dut.uo_out)
